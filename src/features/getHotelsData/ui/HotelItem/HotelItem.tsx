...
import { FavoriteHotelType, HotelType } from '../../model/types/HotelsType'
...
export const HotelItem: React.FC<Props> = memo((props) => {
    const { date, days, favoriteHotels } = props
    const { hotelName, priceAvg, stars, hotelId } = props.hotel
    ...
    const addToFavorites = useCallback(() => {
        if (!favorite) {
            dispatch(hotelsActions.setFavoriteHotel(favHotel))
        } else {
            dispatch(hotelsActions.deleteFavoriteHotel(hotelId))
        }
        setFavorite(!favorite)
    }, [dispatch, hotelId, favorite])

    const goldStars = [...new Array(stars)].map((_, index) => (
        <svg
            key={index}
            width="17"
            height="17"
        ...
        <div className={styles.hotel_item}>
            <div className={styles.info_area}>
                ...
                <div className={styles.hotel_info}>
                    <div className={styles.hotel_name}>{hotelName}</div>
                    <div className={styles.hotel_date}>
                        <span>{date}</span>
                        <div className={styles.dash}></div>
                        <span>
                            {days} {declensionOfNumerals(Number(days), ['день', 'дня', 'дней'])}
                        </span>
                    </div>
            ...
            <div className={styles.price_area}>
                ...
                >
                    <path
                        ...
                        fill={favorite ? '#E55858' : 'white'}
                        stroke={favorite ? '#E55858' : '#C4C4C4'}
                        strokeLinecap="round"
